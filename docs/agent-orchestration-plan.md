...
- 기본 실행/오케스트레이션: Hermes Agent + Codex Pro(월 30만원)
- 선별 투입: Claude Code Pro(월 22만원)
- 원칙: Codex는 넓게 쓰고, Claude Code는 복잡한 UX/TDS/SDK 판단·고품질 카피·최종 리뷰에 쓴다.
- 같은 일을 두 모델에 중복으로 오래 시키지 않는다. Codex가 입력/맥락/체크리스트를 압축하고, Claude Code는 압축된 작업 단위만 받는다.
