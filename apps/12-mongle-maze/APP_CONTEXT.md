# 몽글 미로 탈출 APP_CONTEXT

## 앱 개요

- 경로: `apps/12-mongle-maze`
...
- `npm run lint`
- `npm run build`
- `./node_modules/.bin/vite --host 0.0.0.0 --port 5185 --strictPort` 후 390px 브라우저 QA
...
- 열쇠 외 반짝 조각, 함정 칸, 최단 루트 목표를 추가했다.
- 벽 충돌은 이동 수를 늘리지 않고 점수 페널티와 짧은 피드백으로 처리한다.
- 열쇠+조각+짧은 이동 보너스가 결과 하이라이트/다음 목표에 반영된다.
